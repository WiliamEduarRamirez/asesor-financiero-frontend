...
- **pnpm** como gestor de tareas principal
...
| `core-ui-components`   | Patrones de diseño de HipoExpert UI, uso estricto de componentes `Base*`  | [SKILL.md](skills/core-ui-components/SKILL.md)   |
...
| -------------------------------- | -------------------------------------------- | ---------------------------------------------------------- |
| Escribir componentes UI/Vistas   | `core-ui-components`                         | Utilizar el Design System en `src/core` en lugar de HTML   |
...
- ⚠️ Ejecutar `pnpm type-check` después de cambios estructurales
...
# Calidad de Código
pnpm type-check              # ⚠️ OBLIGATORIO después de cambios profundos de tipos
pnpm lint                    # Verificación de scripts
pnpm format                  # Aplicar reglas de Prettier
```
...
2. **UI**: Cuando el usuario pida interfaces, sigue los estándares de `core-ui-components` con TailwindCSS 4 y el enfoque "Vue 3 Script Setup".
3. **Calidad**: Al terminar un bloque, asegúrate de que pasa `pnpm type-check`.
